...
@Slf4j
@AllArgsConstructor
public class FilesCountProcessor extends AbstractVerticle {
    ...
    public void start() {
        vertx.eventBus().<JsonObject>consumer("words.parsed",
                message -> {
                    log.debug("Ready to count processed files " + message.body());
                    ParsedRecord record = message.body().mapTo(ParsedRecord.class);
                    storage.getProcessedFiles().add(record.getFileName());
                });
    }
}
